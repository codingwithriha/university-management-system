...
<div class="course-form">
    <div class="form-header">
        <h3>✏️ Edit Course</h3>
        <p>Update course information and assignment details</p>
    </div>
    ...
    <form action="{{ route('courses.update', $course->id) }}" method="POST" id="courseForm">
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <div class="input-icon credits">
                    <select name="credits" class="form-select icon-padding" required>
                        ...
                        <option value="5" {{ old('credits', $course->credits) == 5 ? 'selected' : '' }}>5 Credits</option>
                        <option value="6" {{ old('credits', $course->credits) == 6 ? 'selected' : '' }}>6 Credits</option>
                    </select>
                </div>
            ...
            <div class="form-group full-width">
                ...
                <select name="student_ids[]" multiple class="form-select" style="min-height: 150px;">
                    ...
                    <option value="{{ $student->id }}" {{ in_array($student->id, old('student_ids', $enrolledStudentIds)) ? 'selected' : '' }}>
            ...
            <div class="form-group full-width">
                ...
                <div class="description">
                    <textarea name="description"
                        class="form-textarea"
                        rows="6"
                        placeholder="Course description, syllabus, prerequisites...">{{ old('description', $course->description) }}</textarea>
                </div>
        ...
        <div class="course-info">
            ...
            <p><strong>Duration:</strong> {{ $course->duration_hours ?? 'N/A' }} hours</p>
            <p><strong>Status:</strong> <span style="color: '{{ $course->status == 'active' ? '#10b981' : '#ef4444' }}';">{{($course->status) }}</span></p>
            <p><strong>Created:</strong> {{ $course->created_at->format('F j, Y') }}</p>
        </div>
